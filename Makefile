...
GAME_SRCS := src/game/game.c src/game/tower_config.c src/game/creep_config.c
...
test: $(TEST_PATHING_BIN) $(TEST_GAME_BIN) $(TEST_RENDER_BIN) $(TEST_TOWER_CONFIG_BIN) $(TEST_CREEP_CONFIG_BIN)
	$(TEST_PATHING_BIN)
	$(TEST_GAME_BIN)
	$(TEST_RENDER_BIN)
...
$(TEST_PATHING_BIN): $(TEST_PATHING_SRCS) $(TOWER_CONFIG_HDR) $(CREEP_CONFIG_HDR) src/game/game.h src/game/tower_config.h src/game/creep_config.h | build
	cc -O0 -g -Wall -Wextra $(INCLUDES) $(TEST_PATHING_SRCS) -o $(TEST_PATHING_BIN)

$(TEST_GAME_BIN): $(TEST_GAME_SRCS) $(TOWER_CONFIG_HDR) $(CREEP_CONFIG_HDR) src/game/game.h src/game/tower_config.h src/game/creep_config.h tests/test_fixtures.h | build
	cc -O0 -g -Wall -Wextra $(INCLUDES) -Itests $(TEST_GAME_SRCS) -o $(TEST_GAME_BIN)

$(TEST_RENDER_BIN): $(TEST_RENDER_SRCS) $(TOWER_CONFIG_HDR) $(CREEP_CONFIG_HDR) src/game/game.h src/render/render.h src/platform/platform.h tests/test_fixtures.h | build
...
$(OUT): $(SRCS) $(TOWER_CONFIG_HDR) $(CREEP_CONFIG_HDR) $(SHELL_HTML) | build
